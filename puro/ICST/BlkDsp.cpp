...
void BlkDsp::PrepareTransforms()
{
	...
	for (i=1; i<=FFTMAXORD; i++) {			
		ippsFFTInitAlloc_C_32fc(	// (i)fft float
			fftfSpec+i, i, IPP_FFT_DIV_INV_BY_N, ippAlgHintFast	);
		ippsFFTInitAlloc_C_64fc(	// (i)fft double
			fftdSpec+i, i, IPP_FFT_DIV_INV_BY_N, ippAlgHintFast	);
		ippsFFTInitAlloc_R_32f(		// real(i)fft float
			rfftfSpec+i, i, IPP_FFT_DIV_INV_BY_N, ippAlgHintFast	);
		ippsFFTInitAlloc_R_64f(		// real(i)fft double
			rfftdSpec+i, i, IPP_FFT_DIV_INV_BY_N, ippAlgHintFast	);
		ippsDCTFwdInitAlloc_32f(	// dct float
			dctfSpec+i, 1<<i, ippAlgHintFast	);
		ippsDCTFwdInitAlloc_64f(	// dct double
			dctdSpec+i, 1<<i, ippAlgHintFast	);
		ippsDCTInvInitAlloc_32f(	// idct float
			idctfSpec+i, 1<<i, ippAlgHintFast	);
		ippsDCTInvInitAlloc_64f(	// idct double
